...
class ImportDetailTunggakan implements ToCollection
{
    /**
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $key => $value) {
            if ($key!=0) {
                ...
                $insert['nomor_ketetapan'] = $value[18];
                
                $cek_data = DB::table('data.detail_tunggakan')->where('nop', $value[1])->where('npwpd', $value[2])->where('tahun', $value[3])->where('bulan', $value[4])->where('nama_rekening', $value[5])->where('kode_rekening', $value[6])->count();
                if ($cek_data > 0) {
                    DB::table('data.detail_tunggakan')->where('nop', $value[1])->where('npwpd', $value[2])->where('tahun', $value[3])->where('bulan', $value[4])->where('nama_rekening', $value[5])->where('kode_rekening', $value[6])->delete();
                }
                if (!is_null($insert['sumber_data'])) {
                    DB::table('data.detail_tunggakan')->insert($insert);
                }
